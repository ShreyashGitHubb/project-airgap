...
/** Chat message for a structured medical report. */
class ChatMessageMedicalReport(
  val reportTitle: String,
  val metrics: List<MedicalMetric>,
  override val side: ChatSide = ChatSide.AGENT,
  override val accelerator: String = "",
) : ChatMessage(type = ChatMessageType.MEDICAL_REPORT, side = side, accelerator = accelerator)

/** Data class for a single medical metric. */
data class MedicalMetric(
  val name: String,
  val value: String,
...
/** Chat message for a physician discussion handout. */
class ChatMessagePhysicianHandout(
  val title: String,
  val points: List<String>,
  override val side: ChatSide = ChatSide.AGENT,
  override val accelerator: String = "",
) : ChatMessage(type = ChatMessageType.PHYSICIAN_HANDOUT, side = side, accelerator = accelerator)
